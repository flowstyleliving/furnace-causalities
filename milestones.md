...
> Public, curated milestone log for the Furnace research arc — externally-facing, reverse-chronological. A sparse, plain-language timeline of what shipped (distinct from the project's internal raw session log and append-only experimental history). Jargon is defined inline where it helps a non-specialist reader.
...
WRITING CONVENTION (invisible when rendered — keep entries to this shape):
  ...
  → [optional cross-ref](https://...) at the very end (public URLs only — see rule below).
...
Rules:
  ...
  • THIS FILE IS PUBLIC. Cross-refs must resolve publicly — link only to public URLs
    (GitHub repos, arXiv, DOIs, external pages). NEVER link to the private vault's
    .md pages (results/, research-candidates, paper/, *_PLAN.md, etc.) — they 404 here.
    No public target → no link; the prose must stand on its own.
  • Cross-ref last (→ [...](https://...)), never mid-paragraph.
...
### 2026-09-06 — Two of seven measurements were dead

A detector this project has used for months reads attention geometry through seven separate measurements, each taken at three fixed depths in the network. Capturing all seven at every layer for the first time showed that two of them separate correct answers from hallucinated ones no better than a coin flip — at any depth, on any of seventeen model-and-task pairs — and the fitting procedure never selected either across eighty-five independent rounds. They had gone unnoticed because the seven are always averaged together, and five working measurements are enough to make the blend look healthy. An earlier pass over only three of the seven had suggested the panel splits into two distinct kinds of signal; the full set showed that split was an artifact of which three were sampled, and that the strongest single measurement was one nobody had examined. The detector's accuracy is unchanged by either finding. What changes is the accounting: the panel carries less variety, and had received less scrutiny, than its size implied. The practical response is a rule now applied before analysis rather than after: fix the complete list of measurements, comparisons and cut-offs in advance, and check each one alone before trusting any average of them. Three separate findings in this project have dissolved for want of that step, none of them because the underlying data was wrong.
...
Two-act day for the depth-curve line. Act one re-tested the two regularities the full-depth scan had found — the last-layer fade and the one-layer switch-on — under a fraud-proof estimator that selects each model's peak layer on one part of the data and measures the effect only on the held-out remainder: both survived at full strength on the original four models (fade 8 of 8 with margins 2–8× the bar, switch-on 7 of 8), so the earlier rates were not selection artifacts. Act two froze a pre-registration — pass bars, per-family guards, pinned model revisions, hash-pinned prompt manifests, immutable failure records — hardened through nine rounds of adversarial review by an independent model that took the design from red to green, then ran the fade test on six models from three families the regularity had never seen: Llama 3.1 at 8B and 70B, Mistral Small 3.2 and Medium 3.5, Gemma 3 at 12B and 27B.

The fade appeared in 8 of 12 cells — every cell that could be measured except one, permutation p 0.0005 — yet the frozen bar demanded 10 of 12, so the registered verdict is WEAKENED, reported exactly as written. The four misses split into three distinct findings: one model refuses the yes/no answer format outright (a behavioral gate, failed by rule, no rescue); one model breaks the measuring stick itself — Gemma-27B fixates so completely on the first token that the sink-excluding metric has nothing left to measure, meaning the instrument has a domain and this model exits it by its third layer; and exactly one clean, genuine absence. Two registered predictions also landed: the hidden mid-stack band in Llama-70B reappears in a different release of the same 70B model and is absent at 8B — a family trait that grows in with scale, not a quirk of one checkpoint. The practical rule stands: no depth address transfers between models, but one cheap measured pass per model finds where its signal lives. → [depth-curve lane](https://github.com/flowstyleliving/commit-confluence/tree/main/exploratory/depth-curve)

### 2026-08-16 — The depth map had a blind spot

A pre-registered depth scan measured the commitment signal at every layer of four models — Qwen at 7B, 32B and 72B parameters plus Llama-70B, on two benchmarks, 200 examples each, all faithfulness gates clean — where every previous panel had sampled just three depths per model. Both frozen hypotheses about where the signal peaks failed honestly (three of four registered predictions missed, reported as written): the peak sits neither at a fixed offset from the top nor at a fixed fraction of depth, so there is no placement rule that transfers to a new model size without measuring, and the "peak at the second-to-last layer" that the three-point panels had reported dissolves into a resolution artifact. The registered negative control delivered the headline: Llama-70B, which every panel reading had filed as "no attention signal — the readout carries it," turns out to have a broad mid-stack attention band peaking at 0.90, above the readout score that had defined its family story, sitting exactly between the three depths the panels sample — the family dissociation was a property of the map, not the territory. Two regularities survived: the signal switches on in roughly one layer (a cliff, in 7 of 8 cells), and the final layer always gives some of it back — the single most invariant depth fact the project has measured. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)

### 2026-08-12 — The sealed run gets its first executed audit

Every prior independent review of the sealed dispatcher run was static — code read closely, never re-run. That gap is now closed, across all four layers of the chain — data, extraction, statistics, semantics: a hand-trace first verified the data file's checksum and row labels against the profile's stamps, then fresh forward passes on two of the twenty sealed model-task cells, deliberately chosen across different families and benchmarks (Qwen2.5-7B on ANLI, Llama-3.2-3B on TriviaQA), reproduced the stored attention and readout measurements byte-for-byte under preserved seal-time copies of the extraction code — 84 of 84 attention values and 24 of 24 readout values per cell, worst difference exactly zero, every one of the 27 stored measurement types exercised. An independent recomputation of the scoring statistic — plain rank arithmetic, bypassing the project's calibrator entirely — likewise recovered the committed winner's detection score exactly (0.7896, same direction) from the raw stored column. The sealed 18-of-20 verdict is unchanged; what got stronger is the evidence that its numbers mean what they say.

The audit tooling was then adversarially audited itself, over two review rounds, and rebuilt on the findings. The first version printed mismatches and exited clean (fail-open); the published version fails closed: any discrepancy, missing value, or unexplained code drift is a nonzero exit, every run — crashes included — writes a record that binds itself by hash to the exact verifier and artifacts that produced it, and code drift can be waived only by pinning the specific hash of one known file state, a rule negative-tested end to end (a deliberately wrong pin was verified to fail with the failing exit code). The rounds also surfaced one real mystery and chased it to ground: the code hashes recorded in the sealed profiles do not match today's repository — resolved benign, the delta being a documented additive post-seal edit, and re-extraction reproducing the sealed values under both code states doubles as empirical confirmation that the edit touched nothing the seal measured, on every value compared. The published scope statement stays deliberately modest — spot-corroboration of roughly 0.2% of stored values across 2 of 20 cells, not cryptographic attestation — but the standard inside that scope is absolute: byte equality or a failing exit, nothing in between. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)

### 2026-08-03 — First public post: asking for review

The work is out for public critique for the first time — a post to the r/MachineLearning community laying out the central result (no universal hallucination detector, but a universal floor that only partially transfers across models) and inviting researchers to pull it apart. The timing was deliberate rather than incidental: until the pre-registered benchmark extension was finished and its failures published, there was nothing honest enough to put in front of people — an earlier post would have been premature in real ways. There is a small irony in the venue. A LessWrong moderator had banned the account from posting some time back, closing off the more research-forum-native home; Reddit ended up being both the first option and the last one. More on that thread later. → [r/MachineLearning post](https://www.reddit.com/r/MachineLearning/comments/1veu3l1/no_universal_hallucination_detector_but_a/)

The post was up for a fair while before the subreddit's moderators removed it, under the community's rule against LLM-generated writing — on a forum about LLMs. Taken together with the earlier LessWrong ban, the venue question the paragraph above raised — where work like this actually gets a hearing — is now sharper rather than settled. The removal changes nothing about the results, which remain public and reproducible; it changes only where the conversation about them can happen. The lesson is narrow and practical: the writing has to sound like the person, not the tool, even when the tool helped do the work.

### 2026-07-23 — Publishing the benchmark arm that failed

The public repository now carries the full six-task benchmark extension: one transfer task (HaluEval-QA — telling hallucinated from faithful answers to knowledge questions), two registered replication tasks (ANLI R1 and TriviaQA), and three exploratory ones (ANLI R2 plus HaluEval's dialogue and summarization variants). This replaces a README frozen at a pre-verdict state while the finished work sat on an unmerged branch — and the scope was not discretionary: the pre-registration requires that a missed replication bar be reported prominently, and the first write-up covered only the transfer task. Both papers were rewritten to carry all three families, including the results that did not pass — the transfer detector that only partly transfers, and the replication arm a frozen integrity rule scored at 7 of 20 — each detailed in the two entries below.

A provenance manifest covering 117 files — 53 score matrices, 53 fitted profiles, and 11 supporting records — is now verified on every push, checking that the published artifacts match their declared checksums. This is repository coherence: a weaker and more honest guarantee than an execution-time signature would claim. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)

### 2026-07-22 — A hallucination signal that points different ways in different models

A pre-registered test asked whether a single frozen detector — one formula, one direction — could be shipped to a model it had never seen labels for. The benchmark was HaluEval-QA: 500 knowledge questions per model, each paired with one faithful and one hallucinated answer. It could not: 6 of 10 models cleared the bar, short of the required 8. The interesting part is why. The four failures did not lose the signal; they inverted it. Measured on each model's own data, that same fused score — the rank-average of three geometric signals, one apiece from attention routing, residual-stream motion, and readout spread — separates hallucinated from faithful answers in all ten models (0.58 to 0.87, where 0.5 is a coin flip), but in Mistral-7B, Mistral-Nemo, Qwen2.5-7B and Phi-3.5 a high score means hallucination while in the other six it means the opposite — so a detector frozen to the majority direction scores those four confidently backwards, as low as 0.17. Per-model calibration, which is allowed to learn each model's own direction, passed all ten. The polarity is only partly a family trait: both Mistral models flip together and both Llama models hold, but Qwen and Phi each split across generations — a pattern an unrelated diagnostic had already stumbled on in Phi-3.5 versus Phi-4 two months earlier. The reading is not "no usable signal", since the same cell works everywhere once its direction is fit locally, but "no universal orientation", which makes per-model calibration a requirement rather than a convenience. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)

### 2026-07-22 — A frozen scoring rule cost us a result, and we kept it

The same run's replication arm — ANLI R1 and TriviaQA, ten models each — reported 7 of 20 against a bar of 17, and not because the measurement failed. A pre-registered integrity rule flags any test cell in which the model ever produces an unparseable answer, with no error budget; a second rule voids an entire task once three cells are flagged. On TriviaQA, which had been the strongest task in the earlier sealed run, three models tripped the first rule on 1, 1 and 12 rows out of 1,000, which voided all ten of that task's cells — including the seven that were clean and that, measured on their own, still separated hallucination from truth at 0.82 to 0.98; the seven cells the arm did score are therefore all ANLI R1, and the same void rule additionally claimed the exploratory ANLI R2. Softening either rule after seeing the outcome would have turned a pre-registered test into a post-hoc one, so the rules stand and the result is reported as-is, labeled as what it is: a commitment-contract failure, not a signal failure. A revised error budget and a widened definition of an acceptable answer are being drafted for a future pre-registration, to be fixed before that run rather than after it. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)
...
A behavioral measurement across four models, two families, and three scale tiers (7B to 72B parameters) found an unexpected constant: the rate at which two models disagree on the same logic puzzle's first-impulse answer sits at roughly 18.5%, whether those models are from the same family at different sizes (Qwen-7B versus Qwen-32B: 82% agreement) or from different families at matched capability (Llama-70B versus Qwen-72B: 87%). The disagreement ceiling appears to be a property of the task, not the model. A parallel finding: format compliance — whether the model gives a clean YES/NO answer versus drifting into chain-of-thought — is an alignment artifact, not a scale property. Two models at the same 32B parameter tier landed at opposite extremes (Qwen-32B: 100% direct answering, Yi-1.5-34B: 72%), driven by training regime rather than capability. This provides a behavioral-level complement to the signal-level family dissociation reported yesterday: the finding that different families route error-detection signals through different computational pathways (attention morphology versus readout geometry) is not reducible to "different models give different answers." The practical entry point was a methodological control — verifying that cross-precision comparisons are not contaminated by answer-flips, which turned out to be a real but scale-limited concern (the correction matters at 7B, negligible by 32B). A lightweight first-token measurement tool was built and released in the process. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)

### 2026-06-23 — Precision ladder rules out quantization noise

A pre-registered precision ladder tested whether the commitment signal was merely an artifact of low-bit inference. Qwen2.5-7B was run across nf4, int8, bf16, and fp32; Qwen2.5-32B was then run across nf4, int8, and bf16. The robust signal cells kept their direction and strength across precision levels, falsifying the main artifact explanation: the monitor is reading real model computation, not rounding noise. The run also caught a provenance bug — an earlier Qwen-32B baseline labeled as nf4 was actually bf16 — and the corrected true-nf4 rerun still won on attention morphology, which deconfounds the family-locus result at matched precision. A smaller lesson emerged too: int8 degradation and winner-instability show up at 7B but wash out by 32B, so cross-precision comparisons should track fixed cells rather than the noisy argmax winner. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)

### 2026-06-22 — The diagnostic signal lives in different places for different model families

A cloud-based extension of our commitment-diagnostic panel to larger models — run on GPU hardware, complementing the original Apple-Silicon results — surfaced a clean dissociation. For the Qwen model family the predictive signal lives in *attention morphology*: how the model routes its attention over the prompt just before it answers. For the Llama family the very same panel instead finds its signal in *readout geometry*: how the model's output-probability distribution spreads at the first generated token. The two are genuinely different moments in the computation, and which one carries the signal turns out to depend on the model family, not the task. The practical lesson reinforces our central finding: there is no single universal "where to look," but a per-model calibration procedure reliably finds the right place — and a panel that spans both moments, rather than any one signal, is what transfers across models.

The same runs also closed a second open gap from our sealed benchmark. A hard natural-language-inference case that the diagnostic could not flag on a small Llama model becomes reliably detectable once the model is scaled to 70 billion parameters. This mirrors an earlier result on the Gemma family and confirms that both gaps were artifacts of small model size rather than limits of the method. As with all of our cloud extensions, these results use a different numerical implementation than the sealed benchmark and are reported as standalone exploratory points, never merged into the sealed scorecard.

### 2026-06-21 — A gemma blind spot was about model size, not model generation
The one persistent failure in the sealed study — a small gemma model our monitor could not read on the hardest reasoning benchmark (ANLI) — turns out to be a property of model *scale*, not of the gemma family or its generation. Re-running the same pre-registered pipeline on a larger same-generation model (gemma-3 at 12B parameters, versus the failing 4B) flipped that case from undeployable to clearly deployable (detection-score lower bound 0.40 → 0.71), and a same-size model from a different family (Qwen2.5-14B) also passed, ruling out a generic mid-size failure. A newer model *generation*, gemma-4 at 12B, then closed the question from the other side: it too was deployable on both benchmarks (0.69 and 0.75), so the blind spot does not reappear a generation later. Reaching gemma-4 meant reimplementing its attention extraction from scratch against an architecture no released inference tool yet supports — and surfaced a clean measurement-hygiene lesson: the first gemma-4 numbers looked like total failure (≈0.37 everywhere) purely because the model needs its chat template to attempt the task at all; once that was fixed, the signal was there. These extensions do not change the sealed headline result — they localize its single open orphan to small-model capacity specifically. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)

### 2026-06-11 — One honest dispatcher over four signals: geometry holds, confidence is not the safety net
A single calibrated monitor was run that, at the moment a model commits to its first word, fits **all four in-house hallucination signals at once** — attention-routing morphology, residual-stream motion, readout geometry, and ordinary output confidence — and lets an honest, no-peeking selector pick the best one per model and task. On a pre-registered run across 10 models × 2 benchmarks (200 examples each, fresh data committed before the run), the **geometry-only** monitor was deployable on **18 of 20** model-task pairs, clearing its registered bar; the stricter **everything-included** claim missed its higher bar by a single cell. The decisive finding: the full panel and the geometry-only panel failed the **same two** cases, so adding confidence rescued nothing — those two are genuine blind spots that no signal in the panel can see, not places where geometry was weak and confidence covered. And there was **no universal best signal**: 12 different cells won across the 18 deployable cases, making concrete the project's running thesis that hallucination detection must be calibrated per deployment, not solved once. The whole build was hardened through four independent adversarial review passes before launch. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)
...
`pri_calibrator.py` (440 LOC, schema v1.0) operationalizes per-(model, task) PRI rupture detection after the Codex adversarial review showed that the prior label-free meta-classifier claim was inflated — `auroc_signed` was peeking at held-out labels to flip sign. The calibrator takes a small labeled set (n=10-50 prompt+label pairs), traces each sample, sweeps an 8-cell metric panel under direction-preserving scoring with sign locked from the calibration data, bootstraps a 95% CI, and persists a versioned JSON profile with deployability warnings and reproducibility provenance (data hash + pipeline module hash). End-to-end at n=30 on Mistral-Nemo + ANLI R2: picks `d_F_full @ step 1` with AUROC=0.911, CI [0.778, 1.000], sign=−1 — same sign the full 200-sample cross-task analysis found, recovered from 30 labeled samples.
...
Capture-only + descriptive-metric addition: closed-form `kl_discharged`, properly centered Fisher null-ratio (`null_ratio_centered_post_rank{r}` with the `−ppᵀ` correction), and `gen_p_t_topk` capture for replay. New `v3_2_centered_bakeoff` scope runs a three-way Fisher / Raw / Centered-Fisher comparison across six cross-architecture models, seed 20260507. Sealed E18 / E17b primaries unchanged.

### 2026-05-06 — Junjie Hu (HARP first author) responds to PRI v3 outreach
Sent the v3 paper PDF to Hu (Hu et al. 2025, *HARP: Hallucination Detection via Reasoning Subspace Projection*). Reply was warm, engaged, and specific — Hu identified the **"static SVD saturation vs. Fisher reweighting" regime distinction** as "a meaningful extension of the original perspective." Substantive technical feedback still pending his careful read.

### 2026-05-02 — PRI v3 paper draft completed; arXiv submission tracker live
Workshop-length (~8pp) draft, end-to-end prose, n=150 powered numbers, three-motif cross-architecture story, Mistral Simpson's-paradox at r=32, full *Method in Plain Language* section. Endorser outreach to Farquhar / Kalai / Pineau initiated.

### 2026-04-24 / 2026-04-25 — v3.1 sealed-gate replicate confirms PRI v3
Fresh-seed (20260423) replicate of the sealed E18 gate on three primary models (Llama 3.2 3B, Mistral 7B, Qwen 2.5 7B). All three clear AUROC ≥ 0.60 with non-overlap CI vs 0.5 — the 2026-04-23 verdict holds on independent data. Validates the result wasn't a single-seed artifact.

### 2026-04-23 — PRI v3 main-run verdict: sealed gate PASSES at rank 1
First time PRI v3's `null_ratio` (the geometric rupture metric) cleared a pre-registered falsification gate. **3 of 3 primary models pass:** Llama 3.2 3B at AUROC 0.86 [0.81, 0.91], Mistral 7B at 0.86 [0.81, 0.91], Qwen 2.5 7B at 0.73 [0.66, 0.80]. The companion E19 interpretation gate failed across all four tested models — sharpening the open question of whether direction carries signal independent of magnitude.

### 2026-04-14 — PRI v3 plan filed: cross-layer Fisher eigenspace projection
Refined PRI v3 from "eigendecomposed Fisher + `null_ratio`" to per-layer null-projection, yielding a **depth profile per sample** — a new observable PRI v2 cannot express. Six pre-registered falsification conditions, energy-anchored rank reporting, sign-invariance proven (not assumed).
...
Step-1 commitment framing formalized: rupture concentrates at the first generated token, not at encoding. The shift from "rupture happens somewhere in the model" to "rupture happens *here*, measurably."
