...
The same run's replication arm reported 7 of 20 against a bar of 17 — not because the measurement failed. A pre-registered integrity rule flags any test cell in which the model ever produces an unparseable answer, with no error budget; a second rule voids an entire task once three cells are flagged. On what had been the strongest task in the earlier sealed run, three models tripped the first rule on 1, 1 and 12 rows out of 1,000, which voided all ten of that task's cells — including the seven that were clean and that, measured on their own, still separated hallucination from truth at 0.66 to 0.98. Softening either rule after seeing the outcome would have turned a pre-registered test into a post-hoc one, so the rules stand and the result is reported as-is, labeled as what it is: a commitment-contract failure, not a signal failure. A revised error budget and a widened definition of an acceptable answer are being drafted for a future pre-registration, to be fixed before that run rather than after it. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)
