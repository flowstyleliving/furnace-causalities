...
Every prior independent review of the sealed dispatcher run was static — code read closely, never re-run. That gap is now closed: fresh forward passes on two of the twenty sealed model-task cells, chosen across different families and benchmarks (Qwen2.5-7B on ANLI, Llama-3.2-3B on TriviaQA), reproduced the stored attention and readout measurements byte-for-byte — 84 of 84 attention values and 24 of 24 readout values per cell — running under preserved seal-time copies of the extraction code. An independent recomputation of the scoring statistic — plain rank arithmetic, bypassing the project's calibrator entirely — likewise recovered the committed winner's detection score exactly (0.7896, same direction) from the raw stored column. The sealed 18-of-20 verdict is unchanged; what got stronger is the evidence that its numbers mean what they say.

The audit tooling was then adversarially audited itself, over two review rounds, and rebuilt on the findings. The first version printed mismatches and exited clean (fail-open); the published version fails closed: any discrepancy, missing value, or unexplained code drift is a nonzero exit, every run writes a record that binds itself by hash to the exact verifier and artifacts that produced it, and code drift can be waived only by pinning the specific hash of one known file state — a deliberately wrong pin was verified to fail. The rounds also surfaced one real mystery and chased it to ground: the code hashes recorded in the sealed profiles do not match today's repository — resolved benign (the delta is a documented additive post-seal edit), with re-extraction reproducing the sealed values under both code states. The published scope statement stays deliberately modest: spot-corroborating roughly 0.2% of stored values across 2 of 20 cells is reproducibility evidence, not cryptographic attestation — and the repository says exactly that. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)
