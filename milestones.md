...
The public repository now carries the complete six-task benchmark extension: HaluEval-QA — telling hallucinated from faithful answers to knowledge questions — as the registered transfer task, ANLI R1 (adversarial natural-language inference) and TriviaQA as the registered replication pair, and ANLI R2 plus HaluEval's dialogue and summarization variants as exploratory ones. This replaces a README that had been frozen at a pre-verdict state while the finished work sat on an unmerged branch. The expansion was not discretionary: the study's pre-registration contains a clause requiring that if either replication endpoint misses its bar, the write-up must report the miss prominently — and the first draft of the report covered only the transfer task. Both papers were rewritten to carry all three task families: the transfer split on HaluEval-QA (per-model calibration 10 of 10 models, a single frozen detector 6 of 10), the ANLI-R1-and-TriviaQA replication that scored 7 of 20 after a frozen integrity rule voided all ten TriviaQA cells, a probe showing that both previously-undetectable ANLI-R1 cases — gemma-3-4B and Llama-3.1-8B — become detectable at five times the sample size (200 rows to 1,000), and three exploratory tasks carrying no pass/fail bars, one of which (ANLI R2) the same rule voided as well. A provenance manifest covering 117 files — 53 score matrices, 53 fitted profiles and 11 supporting records — is now checked on every push, verifying that the published artifacts match their declared checksums: repository coherence, which is a weaker and more honest claim than an execution-time signature. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)
