...
The public repository now carries the complete six-task benchmark extension, replacing a README that had been frozen at a pre-verdict state while the finished work sat on an unmerged branch. The expansion was not discretionary: the study's pre-registration contains a clause requiring that if either replication endpoint misses its bar, the write-up must report the miss prominently — and the first draft of the report covered only the task that had passed. Both papers were rewritten to carry all three task families: the transfer split on hallucination detection (per-model calibration 10 of 10, a single frozen detector 6 of 10), the replication arm that scored 7 of 20 after a frozen integrity rule voided an entire task, a probe showing that both previously-undetectable inference cases become detectable at five times the sample size, and three exploratory tasks reported with no pass/fail bars attached. A provenance manifest covering 117 files is now checked on every push, verifying that the published artifacts match their declared checksums — repository coherence, which is a weaker and more honest claim than an execution-time signature. → [commit-confluence](https://github.com/flowstyleliving/commit-confluence)
